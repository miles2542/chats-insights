...
interface CallActivityHeatmapProps {
...
const CallActivityHeatmap = ({
	messages,
	timezoneOffset,
	selectedParticipants,
	timeRange,
}: CallActivityHeatmapProps) => {
	const [view, setView] = useState<"heatmap" | "split">("heatmap");
	...
	useEffect(() => {
		...
		const worker = new Worker(
			new URL("../../workers/call-activity-worker.ts", import.meta.url),
		);
		...
		// Filter for calls and pass required fields
		const callMessages = messages
			...
			.map((m) => ({
				...
				callStartTimeMs: m.callStartTimeMs,
				callEndTimeMs: m.callEndTimeMs,
	...
	}, [messages, timezoneOffset, selectedParticipants, timeRange, timeMode]);
	...
	const heatmapOption = useMemo(() => {
		...
		return {
			...
			visualMap: {
				type: "continuous",
				min: 1,
				max: Math.max(...data.heatmapData.map((d) => d[2]), 1),
				seriesIndex: [1],
				calculable: true,
				orient: "vertical",
				right: "1%",
				...
				<div className="flex border border-[#EAE8E3] dark:border-[#333] bg-[#F5F5F5] dark:bg-[#111] p-0.5">
					{(["Start", "End"] as const).map((m) => (
						<button
							...
							className={`px-3 py-1 text-[9px] uppercase font-bold transition-colors ${timeMode === m ? "bg-[#111] text-white dark:bg-[#EAE8E3] dark:text-[#111]" : "text-[#888] hover:text-[#111] dark:hover:text-white disabled:opacity-50"}`}
						>
							Call {m}
						</button>
					))}
...
export default memo(CallActivityHeatmap);
