...
import type {
	SocialCircleWorkerResponse,
	ThreadData,
} from "../../../workers/social-circle-worker";
...
export default function SocialCirclePage() {
	...
	const handleFilesSelected = (e: React.ChangeEvent<HTMLInputElement>) => {
		...
		setTimeout(() => {
			...
			const allThreads: Record<string, ThreadData> = JSON.parse(
			...
			for (let i = 0; i < numWorkers; i++) {
				...
				worker.onmessage = (
					event: MessageEvent<SocialCircleWorkerResponse>,
				) => {
					...
					if (res.type === "PROGRESS") {
						...
						Object.entries(res.threads).forEach(([name, data]) => {
							if (!allThreads[name]) {
								...
								t.timestamps = Array.from(
									new Set([...t.timestamps, ...data.timestamps]),
								).sort((a, b) => a - b);
								t.daysActive = Array.from(
									new Set([...t.daysActive, ...data.daysActive]),
								).sort();
								Object.entries(data.catCounts).forEach(
									([k, v]) => (t.catCounts[k] = (t.catCounts[k] || 0) + v),
								);
								Object.entries(data.msgStats.types).forEach(
									([k, v]) =>
										(t.msgStats.types[k] = (t.msgStats.types[k] || 0) + v),
								);
								t.msgStats.totalWords += data.msgStats.totalWords;
								t.msgStats.emojiCount += data.msgStats.emojiCount;
								data.heatmap.forEach((row, d) =>
									row.forEach((v, h) => (t.heatmap[d][h] += v)),
								);
								...
								Object.entries(data.timelineCounts).forEach(([day, v]) => {
									if (!t.timelineCounts[day])
										t.timelineCounts[day] = { total: 0, senders: {} };
									t.timelineCounts[day].total += v.total;
									Object.entries(v.senders).forEach(
										([s, count]) =>
											(t.timelineCounts[day].senders[s] =
												(t.timelineCounts[day].senders[s] || 0) + count),
									);
								});
								Object.entries(data.peakTrackers).forEach(([type, tracker]) => {
									Object.entries(tracker).forEach(
										([k, v]) =>
											(t.peakTrackers[type][k] =
												(t.peakTrackers[type][k] || 0) + v),
									);
								});
							}
						});
	...
	const data = useMemo(() => {
		...
		const totalCount = filteredThreads.reduce(
			(acc, t) => acc + t.totalCount,
			0,
		);
		...
		filteredThreads.forEach((t: ThreadData) => {
			...
			Object.entries(t.catCounts).forEach(
				([k, v]: [string, number]) => (catCounts[k] = (catCounts[k] || 0) + v),
			);
			Object.entries(t.msgStats.types).forEach(
				([k, v]: [string, number]) =>
					(msgStatsTypes[k] = (msgStatsTypes[k] || 0) + v),
			);
			...
			const mergeDetails = (
				...
				Object.entries(source).forEach(([key, senders]) => {
					...
					Object.entries(senders).forEach(
						([s, v]) => (target[key][s] = (target[key][s] || 0) + v),
					);
				});
			};
			...
			Object.entries(t.timelineCounts).forEach(
				([day, dData]: [string, any]) => {
					if (!dayBuckets[day]) dayBuckets[day] = { total: 0, senders: {} };
					dayBuckets[day].total += dData.total;
					Object.entries(dData.senders as Record<string, number>).forEach(
						([s, v]) => {
							dayBuckets[day].senders[s] =
								(dayBuckets[day].senders[s] || 0) + v;
						},
					);
				},
			);

			Object.entries(t.peakTrackers).forEach(
				([type, tracker]: [string, any]) => {
					Object.entries(tracker as Record<string, number>).forEach(
						([k, v]) => {
		...
		const inactivityPercentage =
			totalRangeDays > 0
				? ((daysWithoutActivity / totalRangeDays) * 100).toFixed(1)
				: "0";
		const activityPercentage =
			totalRangeDays > 0
				? ((daysActiveCount / totalRangeDays) * 100).toFixed(1)
				: "0";
		...
		const aggregatedTimeline: Record<
			string,
			{ total: number; senders: Record<string, number> }
		> = {};
			...
			while (curr <= end) {
				...
				if (dayData) {
					...
					Object.entries(dayData.senders).forEach(([s, v]) => {
						aggregatedTimeline[aggKey].senders[s] =
							(aggregatedTimeline[aggKey].senders[s] || 0) + v;
					});
				}
		...
		return {
			...
			totalRangeDays,
			activityPercentage,
			categoryCounts: Object.entries(catCounts)
			...
			hourData: Array.from({ length: 24 }, (_, h) =>
				heatmap.reduce((acc, row) => acc + row[h], 0),
			),
	...
	const totalProgress =
		Object.values(workerProgress).reduce(
			(acc, curr) => acc + curr.current / curr.total,
			0,
		) / (Object.keys(workerProgress).length || 1);
		...
		<div className="grid grid-cols-12 gap-6 animate-in fade-in duration-500 pb-32">
			...
			<div className="col-span-12 space-y-8">
				<div className="text-center space-y-4">
					...
					<p className="text-[10px] uppercase font-bold tracking-[0.2em] text-[#888] max-w-3xl mx-auto leading-relaxed">
						Holistic view of your digital social sphere. Upload your entire{" "}
						<span className="bg-[#111] text-white dark:bg-[#EAE8E3] dark:text-[#111] px-1.5 py-0.5 rounded-sm">
							inbox
						</span>{" "}
						folder for complete network mapping. Data remains local.
					</p>
				</div>
									...
									<>
										...
										<div className="flex flex-col items-center gap-1">
											<div className="text-[8px] uppercase font-bold text-[#888]">
												Processing {Object.keys(workerProgress).length} threads
												in parallel
											</div>
											<div className="text-[8px] font-mono text-[#D93829] truncate max-w-xs">
												{Object.values(workerProgress)
													.map((p) => p.fileName)
													.find((f) => f) || "Initializing..."}
											</div>
										</div>
									</>
								)}
				...
				<div className="flex flex-wrap justify-center gap-4 mt-6">
					...
					{Object.keys(rawThreads).length > 0 && (
						<button
							onClick={() =>
								setSocialCircleData({ rawThreads: {}, isInitialized: false })
							}
				...
				<div className="flex justify-center">
					<label className="flex items-center gap-3 cursor-pointer group">
						<div className="relative">
							...
							/>
							<div
								className={`w-5 h-5 border-2 transition-all ${excludeGroupChats ? "bg-[#D93829] border-[#D93829]" : "border-[#ccc] dark:border-[#444] group-hover:border-[#D93829]"}`}
							>
								{excludeGroupChats && (
									<svg
										viewBox="0 0 24 24"
						...
						<span
							className={`text-[10px] uppercase font-black tracking-widest transition-colors ${excludeGroupChats ? "text-[#D93829]" : "text-[#555] dark:text-[#aaa]"}`}
						>
			...
			<div className="col-span-12 grid grid-cols-4 gap-6 mb-6">
				{[
					[
						"Total Messages",
						isDataLoaded ? data?.filteredCount?.toLocaleString() : "---",
					],
					[
						"Total Chats",
						isDataLoaded ? (
							<div className="flex items-baseline gap-1.5">
								...
								<div className="flex items-center gap-1 text-[10px] font-bold text-[#888] lowercase">
									({data?.participantsCount?.toLocaleString()} unique
									participants)
					...
					<div
						key={label as string}
						className={`p-6 shadow-sm border ${idx === 0 ? "bg-[#111] text-[#EAE8E3] dark:bg-[#EAE8E3] dark:text-[#111] border-transparent" : "bg-white dark:bg-[#1A1A1A] border-[#EAE8E3] dark:border-[#333]"}`}
					>
						<div
							className={`text-[10px] uppercase tracking-widest font-black mb-2 ${idx === 0 ? "text-[#888] dark:text-[#555]" : "text-[#888]"}`}
						>
							{label as string}
						</div>
						<div
							className={`text-4xl font-[family-name:var(--font-outfit)] font-bold ${idx !== 0 ? "text-[#D93829]" : ""}`}
						>
							{val}
						</div>
					</div>
					...
					{isDataLoaded ? (
						<div className="space-y-4">
							...
							<div className="pt-4 border-t border-[#EAE8E3] dark:border-[#333] space-y-2">
								{[
									...
									[
										...
										<InfoTooltip
											key="inactive"
											content="Days with zero incoming or outgoing activity across the entire archive range."
										/>,
										<span
											key="pct"
											className="text-[9px] text-[#888] font-normal lowercase ml-1"
										>
									...
									>
										<span className="text-[9px] uppercase font-bold text-[#888] flex items-center gap-1">
											{l as string}
											{tooltip as any}
										</span>
							...
							<div className="pt-4 border-t border-[#EAE8E3] dark:border-[#333]">
								<span className="text-[9px] uppercase font-black text-[#D93829] block mb-2">
									Most Active
								</span>
									...
									{[
										...
										>
											<span className="text-[#888]">
												{label as string}
												<InfoTooltip
													content={`${(p as any).val.toLocaleString()} messages`}
												/>
											</span>
											<span className="truncate ml-4">
												{type === "year"
													...
													: type === "month"
														? formatDate(
																new Date(`${(p as any).key}-01`).getTime(),
																timezoneOffset,
																"month",
															)
														: type === "day"
															? formatDate(
																	new Date((p as any).key).getTime(),
																	timezoneOffset,
																	"date",
																)
															: renderSubtleDate(
